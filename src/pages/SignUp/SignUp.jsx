  ...
  const onSubmit = (data) => {
    createUser(data.email, data.password).then((result) => {
      updateUserProfile(data.name, data.photoURL).then(() => {
        // Add a default Bronze badge
        const userInfo = {
          name: data.name,
          photoURL: data.photoURL,
          email: data.email,
          badge: "Bronze", // Assign default badge
        };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="card-body py-0">
              ...
              <div className="form-control">
                ...
                {errors.password?.type === "minLength" && (
                  <span className="text-red-600">6 char required</span>
                )}
                {errors.password?.type === "maxLength" && (
                  <span className="text-red-600">20 char exceeded</span>
                )}
